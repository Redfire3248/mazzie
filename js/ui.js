...
function updateMenuProfile() {
  ...
  if (offlineEl) {
    const isOffline = currentAccount && (currentAccount.offline || currentAccount.local);
    offlineEl.style.display = isOffline ? '' : 'none';
  }
}
...
function openNameEdit() {
  ...
  setTimeout(() => {
    ...
    i.style.touchAction = 'auto'; i.focus();
  }, 300);
}

// Live availability check as user types (for name-edit screen)
...
async function saveName() {
  ...
  // Global rename via account system
  ...
  // Also check lobby uniqueness if in a room
  ...
  if (inLobby) {
    ...
    if (taken) { errEl.innerText = 'Name already taken in this room!'; return; }
  }
...
// ══════════════════════════════════════════════════
// ADMIN — UID whitelist (no PIN for whitelisted UIDs, silent for everyone else)
// ══════════════════════════════════════════════════
function isAdminUid() {
  ...
  // Match against full UID OR the short form (#XXXXXXXX = last 8 upper)
  const short = uid.slice(-8).toUpperCase();
  return list.some(a => a === uid || a.replace('#','').toUpperCase() === short);
}

function adminOpen() {
  if (!isAdminUid()) return;   // not in whitelist — do nothing, no hint
  openAdminPanel();            // whitelisted — open directly, no PIN
}
...
// PIN functions kept for the "Change PIN" button inside the panel (optional legacy)
function pinKey(k) {
...
function openAdminPanel() {
  ...
  const uidDisp = document.getElementById('adm-my-uid');
  if (uidDisp) {
    uidDisp.innerText = '#' + uid.slice(-8).toUpperCase();
    uidDisp.title = uid; // hover shows full UID
    uidDisp.onclick = () => {
      navigator.clipboard.writeText(uid).catch(() => {});
      pushToast('UID copied: ' + uid, 'acc');
    };
  }
  adminRefreshStatus();
...
function renderAdminTargets() {
  ...
  Object.entries(lobbyPlayers).forEach(([pid, p]) => {
    ...
    // Show name + short UID
    const shortUid = '#' + pid.slice(-6).toUpperCase();
    chip.innerHTML = `<span>${p.name}${pid===myId?' (you)':''}</span><span class="adm-chip-uid">${shortUid}</span>`;
...
// F2 — admins only. Toggle open/close. Silent for everyone else.
document.addEventListener('keydown', e => {
  if (e.key !== 'F2') return;
  e.preventDefault();
  if (!isAdminUid()) return; // silent — non-admins get no feedback
  const panelOpen = !document.getElementById('admin-panel').classList.contains('adm-hidden');
  if (panelOpen) adminClose();
  else openAdminPanel();
});
