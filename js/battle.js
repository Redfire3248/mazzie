...
function doJoin() {
  ...
  conn.on('open', () => {
    ...
    conn.send({ type:'join', id:myId, name:myName, xpLevel:getXpLevel(loadSave().xp||0), rankName:getRank(cleared).name });
    document.getElementById('join-status').innerText = 'Connected! Waiting for host…';
  });
  conn.on('data', d => handleHostMsg(d));
  conn.on('close', () => { if (battleActive) pushToast('Disconnected from host','warn'); hostConn = null; });
  conn.on('error', () => { document.getElementById('join-status').innerText = 'Could not connect. Check the code.'; hostConn = null; });
}
...
function leaveLobby() {
  ...
  isHost = false; battleActive = false; lobbyPlayers = {}; roomCode = '';
  show('battle-mode');
}
...
function hostStart() {
  ...
  level = 1; // FIX: reset level so host board matches guests
...
function handleHostMsg(d) {
  if (d.type === 'lobby_update') {
    ...
    renderLobby();
    // FIX: navigate to lobby if we're still on join screen
    const joinScreen = document.getElementById('join-screen');
    if (joinScreen && !joinScreen.classList.contains('hidden')) {
      show('lobby');
      document.getElementById('host-code-box').style.display = 'none'; // guests don't see room code box
...
function showFinalResults() {
  ...
  document.getElementById('fin-winner').innerText =
    sorted.length > 0 ? (lobbyPlayers[sorted[0][0]]?.name || sorted[0][0]) + ' wins! 🎉' : '';
  ...
  sorted.forEach(([pid, pts], i) => {
    ...
    row.innerHTML = `<div class="fin-medal">${MEDALS[i]||'#'+(i+1)}</div>
      <div class="fin-info">
        ...
        <div class="fin-score">${Object.keys(roundScores).length} rounds played</div>
      </div>
...
function playAgain() {
  ...
  roundScores = {}; finishOrder = []; battleRound = 0; quitPlayers.clear(); progressState = {};
